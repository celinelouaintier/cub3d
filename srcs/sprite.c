...
/*   Updated: 2025/02/03 21:58:27 by clouaint         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_sprite_line(t_data *data, int i, int tex_x, int stripe)
{
	...
	while (y++ < data->sprite.draw_end_y)
	{
		...
		tex_y = ((d * data->enemy[data->current_sprite].height)
				/ data->sprite.sprite_height) / 256;
		data->img.color = data->enemy[data->current_sprite]
			.addr[data->enemy[data->current_sprite].width * tex_y + tex_x];
		if (data->img.color)
		{
...
void	draw_sprite(t_data *data, int i)
{
	...
	while (stripe++ < s->draw_end_x)
	{
		tex_x = (int)(256 * (stripe - (-s->sprite_width
						/ 2 + s->screen_x))
				* data->enemy[data->current_sprite].width
				/ s->sprite_width) / 256;
